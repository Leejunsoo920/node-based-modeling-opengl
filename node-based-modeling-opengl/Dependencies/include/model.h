...
public:
    ...
    void move_vertex(
        ...
        int SCR_HEIGHT,
        glm::vec3 cameraPos,
        int mim_distance_num)
    {


        //meshes[0].vertices[mim_distance_num].Position[0] = inputctrl_global->mouse_pos_wolrd_pos[0];
        //meshes[0].vertices[mim_distance_num].Position[1] = inputctrl_global->mouse_pos_wolrd_pos[1];
        //meshes[0].vertices[mim_distance_num].Position[2] = inputctrl_global->mouse_pos_wolrd_pos[2];
...
private:
    ...
    void loadModel(std::string const& path)
    {
        ...
        Assimp::Importer importer; // Assimp에서는 한줄의 코드로 모델정보를 다 입력할 수 있도록 처리가능
        // Triangulate는 삼각형 구성이 안되있을 경우 삼각형 변환
        // FlipUVs는 처리중 필요한 경우 y축 텍스쳐 뒤집기 ( opengl은 대부분 텍스쳐 반전)
