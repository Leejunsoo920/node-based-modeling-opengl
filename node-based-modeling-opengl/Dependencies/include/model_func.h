        ...
        vector<int> &in_sq)
    {
        ...
        for (unsigned int i = 0; i < vertices.size(); ++i)
        {
            ...
            if (screen_pos[0] > x_min &&
                ...
                screen_pos[1] < y_max)
            {
                if (find(in_sq.begin(), in_sq.end(), i) == in_sq.end())
                {
                    in_sq.push_back(i);
                }
            }
...
public:
    ...
    void check_piking_distance(int mouse_pos[2], vector<glm::vec3> vertices)
    {
    ...
    void set_pick_select_vertex(
        vector<Vertex> vertices,
        vector<int>& in_select_vertex)
    {
        if (find(in_select_vertex.begin(), in_select_vertex.end(), picking_vertex_num) == in_select_vertex.end())
        {
