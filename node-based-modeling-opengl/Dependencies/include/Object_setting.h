...
public:
	...
	Object_setting(const char* path,const char* name, bool gamma = false) : object(path, gamma), name(name){}
	//// to deep copy
		...
		const glm::vec3& camera_position)
	{
		...
		else 
		{
			...
			/*
			for (unsigned int i = 0; i < object.meshes[0].vertices.size(); i++)
			{
				debug.set_text("pos"+std::to_string(i), object.meshes[0].vertices[i].Position);
				debug.set_text("uv" + std::to_string(i), object.meshes[0].vertices[i].TexCoords2);
			}
			*/
		}
