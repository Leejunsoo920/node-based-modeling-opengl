...
public:
    ...
    void set_pre_animation(const char* path,const char* name, bool gamma = false)
    {
        ...
        //// get animation
        std::shared_ptr<Animation> new_animation = std::make_shared<Animation>(path, new_object->object.m_BoneInfoMap, new_object->object.m_BoneCounter);
...
private:    
    ...
    void select_gen_node(objects_map_t& pre_object_map);
    void imgui_render();
    ...
    void check_vertex_num(const std::vector<glm::vec3> & vertices)
    {
        Vertex_Picking vertex_picking;
        vertex_picking.check_piking_distance(inputctrl_global->mouse_pos, vertices);
        inputctrl_global->check_vertex_num = vertex_picking.picking_vertex_num;
    ...
    void instancing_draw(
        ...
        std::vector<glm::mat4>& model_M, // to clear model_M in this function, call by refernce
        std::vector<glm::vec3>& color_M, // to clear color_M, call by refernce
        const glm::mat4& projection,
        const glm::mat4& view)
    {
